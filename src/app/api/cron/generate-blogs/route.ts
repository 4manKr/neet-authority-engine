...
Generate a comprehensive, SEO-optimized blog post. Return ONLY valid JSON (no markdown fences, no trailing commas, no single-quoted strings, no invalid escape sequences — use \\n for newlines inside strings):
{
  ...
  "imagePrompts": {
    "thumbnail": "FILL THIS WITH AN ACTUAL DALL-E PROMPT (40-80 words) that visually represents THIS specific blog. Rules: indoor setting only, single subject or clean object scene, no crowds. Example for a cutoffs blog: 'Close-up of a young Indian student reviewing a printed NEET rank card at a tidy wooden desk, laptop open in background showing a college comparison table, warm study lamp, shallow depth of field, photorealistic, no text overlays'. Now write the actual prompt for THIS blog topic:",
    "inline1": "FILL THIS WITH AN ACTUAL DALL-E PROMPT (40-70 words) for an inline image that illustrates a KEY CONCEPT from this specific blog. Indoor or flat-lay only. No crowds. Different subject from thumbnail. Example: 'Flat-lay on a white desk: NEET application form, highlighter, organised notes, a small calendar marked with important dates, bright natural light from window, top-down product photography, no text on papers'. Now write the actual prompt for THIS blog's key concept:",
    "inline2": "FILL THIS WITH AN ACTUAL DALL-E PROMPT (40-70 words) for a SECOND inline image showing a different aspect of this blog topic. Indoor counselling or study scene. Completely different composition from thumbnail and inline1. Example: 'Indian medical college admissions counsellor seated at a clean desk, pointing to a laptop screen showing seat allotment details, one student leaning in to look, modern office, natural daylight, professional candid shot, no text visible'. Now write the actual prompt for THIS blog's second visual:"
  }
}`;
